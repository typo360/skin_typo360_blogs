...
namespace EXL\SkinTypo360Blogs\Provider;
...
class FileProvider implements DataProviderInterface {
	private static $backendLayoutFilesPath = 'EXT:skin_typo360_blogs/Configuration/TypoScript/BackendLayouts';
	...
	protected function getBackendLayoutFiles() {
		$directory = GeneralUtility::getFileAbsFileName(self::$backendLayoutFilesPath);
	...
	protected function getTitle($fileInformation) {
		...
		$translatedTitle = $this->getLanguageObject()->sL('LLL:EXT:skin_typo360_blogs/Resources/Private/Language/BackendLayouts.xlf:' . $title);
		if ($translatedTitle) {
	...
	protected function addIcon($fileInformation, BackendLayout $backendLayout) {
		$file = ExtensionManagementUtility::extPath('skin_typo360_blogs', '/Resources/Public/Backend/Images/' . $fileInformation['filename'] . '.');
		$extensions = array('png', 'gif');
		foreach ($extensions as $extension) {
			if (is_file($file . $extension)) {
				$backendLayout->setIconPath(
					ExtensionManagementUtility::extRelPath('skin_typo360_blogs') .
